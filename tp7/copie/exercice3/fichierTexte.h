...
struct UnFichierTexte
{
    ...
    UnModeOuverture modeOuverture; // Précise le mode d'ouverture : consultation, creation, ...
...
/*pour les modes creation et extension, le contenu du paramètre item
  ...
  ATTENTION
  Cette version surchargée de la procédure est nécessaire pour pouvoir prendre en charge l'appel :
  ecrire(monFichier, "bonjour");
    car, par défaut, les constantes littérales chaînes sont considérées comme des bool.
         car C++ considère la constante string littérale comme un char* ...  puis comme bool (cf. document stackOverflow ci-joint)

  Si on ne met pas à disposition cette procédure, il faudrait forcer le typage de la constante littérale string au moment
  de l'appel de la procédure ecrire(  UnFichierTexte& f, string item) de la manière suivante :
  ecrire (monFichier, (string) "bonjour");

  Ce problème ne se pose pas lorsque la chaine se trouve dans une variable string préalablement déclarée.
  string maChaine = "bonjour";
  ecrire (monFichier, maChaine);  ... fait appel à la bonne procédure surchargée ayant le paramètre string item.
